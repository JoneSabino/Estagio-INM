...
def getNames():
    ...
    if file_name == []:
        print('The repository is empty. A new verification will start in 5 minutes.')
        exit(0)
    return share_name, file_name

def shareExists():
    share_name, file_names = getNames()
    if file_service.exists(share_name):
        return True
    else:
        return False
...
def verifyFiles():
    ...
    for file_name in file_names:
        ...
        if file_name == 'resultado_CPU.csv':
            ...
            azurestorage_text = readFile(share_name,file_name)
            dbInsertion(file_name, azurestorage_text)
            ...
            dbInsertion(file_name, azurestorage_text)
    for file_name in file_names:
        delAzFile(share_name, file_name)
    print('\nNo more files to process, a new verification will start in 5 minutes')
...
def readFile(share_name,file_name):
    ...
    return parseFile(azurestorage_text)

rc = 0
def parseFile(azurestorage_text):
    global rc
    dados = []
    # print('\nParsing file...')
...
def dbInsertion(flag, azurestorage_text):
    # cursor = conn.cursor()
    cursor = dbCursor()
    ...
    if flag == 'resultado_CPU.csv':
    ...
    elif flag == 'resultado_disco.csv':
        # print('\nWriting data to SQL Azure...')
        for line in enlist(dados):
            cursor.execute("INSERT INTO disk_metrics(data_, hits, misses, p90_mb) VALUES " + line)
        cursor.commit()
        end = time.clock()
        cursor.close()
        sec = end - start
        ...
        h, m = divmod(m, 60)
        # print('Writing time: %d:%02d:%02d' % (h, m, s))
    elif flag == 'resultado_rede.csv':
        # print('\nWriting data to SQL Azure...')
        for line in enlist(dados):
            cursor.execute("INSERT INTO network_metrics(data_, max_mb, avg_mb, p90_mb) VALUES " + line)
        cursor.commit()
        end = time.clock()
        cursor.close()
        sec = end - start
        ...
        h, m = divmod(m, 60)
        # print('Writing time: %d:%02d:%02d' % (h, m, s))
    elif flag == 'resultado_memoria.csv':
        # print('\nWriting data to SQL Azure...')
        for line in enlist(dados):
            cursor.execute("INSERT INTO memory_metrics(data_, max_mb, avg_mb, p90_mb) VALUES " + line)
        cursor.commit()
        end = time.clock()
        cursor.close()
        sec = end - start
        ...
        print('There\'s no table to this file')

    print('File: ' + flag)
    print('Rows: {:,}'.format(rc))
    print('Writing time: %d:%02d:%02d\n' % (h, m, s))


def delAzFile(share_name, file_name):
